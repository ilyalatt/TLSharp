...
namespace TLSharp.Rpc.Types
{
    public sealed class InputDocument : ITlType, IEquatable<InputDocument>, IComparable<InputDocument>, IComparable
    {
        ...
        public bool Equals(InputDocument other) => !ReferenceEquals(other, null) && (ReferenceEquals(this, other) || CmpPair == other.CmpPair);
        public override bool Equals(object other) => other is InputDocument x && Equals(x);
        public static bool operator ==(InputDocument x, InputDocument y) => x?.Equals(y) ?? ReferenceEquals(y, null);
        public static bool operator !=(InputDocument x, InputDocument y) => !(x == y);

        public int CompareTo(InputDocument other) => ReferenceEquals(other, null) ? throw new ArgumentNullException(nameof(other)) : ReferenceEquals(this, other) ? 0 : CmpPair.CompareTo(other.CmpPair);
        ...
        public static bool operator <=(InputDocument x, InputDocument y) => x.CompareTo(y) <= 0;
        public static bool operator <(InputDocument x, InputDocument y) => x.CompareTo(y) < 0;
        public static bool operator >(InputDocument x, InputDocument y) => x.CompareTo(y) > 0;
        public static bool operator >=(InputDocument x, InputDocument y) => x.CompareTo(y) >= 0;
        ...
        public override string ToString() => $"InputDocument.{_tag.GetType().Name}{_tag}";
    }
}