using System;
using System.IO;
using BigMath;
using LanguageExt;
using static TLSharp.Rpc.TlMarshal;
using T = TLSharp.Rpc.Types;

namespace TLSharp.Rpc.Types.Auth
{
    public sealed class CheckedPhone : ITlType, IEquatable<CheckedPhone>, IComparable<CheckedPhone>, IComparable
    {
        public sealed class Tag : Record<Tag>, ITlTypeTag
        {
            uint ITlTypeTag.TypeNumber => 0x811ea28e;
            
            public bool PhoneRegistered { get; }
            
            public Tag(
                bool phoneRegistered
            ) {
                PhoneRegistered = phoneRegistered;
            }
            
            void ITlSerializable.Serialize(BinaryWriter bw)
            {
                Write(PhoneRegistered, bw, WriteBool);
            }
            
            internal static Tag DeserializeTag(BinaryReader br)
            {
                var phoneRegistered = Read(br, ReadBool);
                return new Tag(phoneRegistered);
            }
        }

        readonly ITlTypeTag _tag;
        CheckedPhone(ITlTypeTag tag) => _tag = tag ?? throw new ArgumentNullException(nameof(tag));

        public static explicit operator CheckedPhone(Tag tag) => new CheckedPhone(tag);

        void ITlSerializable.Serialize(BinaryWriter bw)
        {
            WriteUint(bw, _tag.TypeNumber);
            _tag.Serialize(bw);
        }

        internal static CheckedPhone Deserialize(BinaryReader br)
        {
            var typeNumber = ReadUint(br);
            switch (typeNumber)
            {
                case 0x811ea28e: return (CheckedPhone) Tag.DeserializeTag(br);
                default: throw TlTransportException.UnexpectedTypeNumber(actual: typeNumber, expected: new uint[] { 0x811ea28e });
            }
        }

        T Match<T>(
            Func<T> _,
            Func<Tag, T> tag = null
        ) {
            if (_ == null) throw new ArgumentNullException(nameof(_));
            switch (_tag)
            {
                case Tag x when tag != null: return tag(x);
                default: return _();
            }
        }

        public T Match<T>(
            Func<Tag, T> tag
        ) => Match(
            () => throw new Exception("WTF"),
            tag ?? throw new ArgumentNullException(nameof(tag))
        );

        public bool Equals(CheckedPhone other) => !ReferenceEquals(other, null) && _tag.Equals(other._tag);
        public override bool Equals(object obj) => obj is CheckedPhone x && Equals(x);
        public static bool operator ==(CheckedPhone a, CheckedPhone b) => a?.Equals(b) ?? ReferenceEquals(b, null);
        public static bool operator !=(CheckedPhone a, CheckedPhone b) => !(a == b);

        int GetTagOrder()
        {
            switch (_tag)
            {
                case Tag _: return 0;
                default: throw new Exception("WTF");
            }
        }
        (int, object) CmpPair => (GetTagOrder(), _tag);

        public int CompareTo(CheckedPhone other) => !ReferenceEquals(other, null) ? CmpPair.CompareTo(other.CmpPair) : throw new ArgumentNullException(nameof(other));
        int IComparable.CompareTo(object other) => other is CheckedPhone x ? CompareTo(x) : throw new ArgumentException("bad type", nameof(other));
        public static bool operator <=(CheckedPhone a, CheckedPhone b) => a.CompareTo(b) <= 0;
        public static bool operator <(CheckedPhone a, CheckedPhone b) => a.CompareTo(b) < 0;
        public static bool operator >(CheckedPhone a, CheckedPhone b) => a.CompareTo(b) > 0;
        public static bool operator >=(CheckedPhone a, CheckedPhone b) => a.CompareTo(b) >= 0;

        public override int GetHashCode() => CmpPair.GetHashCode();
    }
}