...
{
    static class TlSchemeNormalizer
    {
        ...
        static TlType Normalize(TlType type) => type.Match(
            primitive: _ => type,
            typeRef: x => x.Name
                .Apply(NormalizeName)
                .Apply(s => NamespaceConflicts.Contains(s) ? s + "Type" : s)
                .Apply(SomeExt.ToSome).Apply(TlType.OfTypeRef),
            vector: x => x.Type.Apply(Normalize).Apply(SomeExt.ToSome).Apply(TlType.OfVector)
        );
