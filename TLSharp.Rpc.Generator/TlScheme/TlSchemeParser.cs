...
{
    static class TlSchemeParser
    {
        static TlSchemeParserException Exception(string msg)
            => new TlSchemeParserException(msg);
        ...
        enum SectionType { Types, Functions }
        ...
        static IEnumerable<(SectionType, string)> SplitBySections(string s)
        {
            ...
            while (true)
            {
                ...
                if (sectionEndIdx == -1) throw Exception("can not find a closing section mark");
        ...
        static TlType ParseType(string s)
        {
            const string vector = "vector";
            string GetVectorType(string ss) => ss.Substring(vector.Length).Apply(x => x.Substring(1, x.Length - 2));
            if (s.StartsWith(vector, true, null)) return s.Apply(GetVectorType).Apply(ParseType).ToSome().Apply(TlType.OfVector);
            ...
            return TryParsePrimitive().ToOption().Map(TlType.OfPrimitive).IfNone(() => TlType.OfTypeRef(s));
        }
        ...
        public static Scheme Parse(Some<string> tlScheme)
        {
            var sections = SplitBySections(tlScheme.Value);
            var version = ExtractLayerVersion(tlScheme);
